...
export default async function TimelinePage({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-zinc-500">谁 · 何时 · 做了什么 · 来源（Web/MCP/Human/System）。</p>
      ...
      <ol className="mt-6 space-y-3">
        ...
        {events.length === 0 && <li className="text-sm text-zinc-400">暂无事件。</li>}
      </ol>
    </div>
  );
}
  ...
  return (
    <li className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <span className={`rounded-full px-2 py-0.5 font-medium ${sourceStyle[e.source] ?? ""}`}>{e.source}</span>
